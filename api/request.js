// api/request.js
// Build & deploy from a brief (debug-tolerant, production-ready).
// - Accepts JSON body (preferred), text containing JSON, or form-urlencoded.
// - Verifies STUDENT_SECRET (from body.secret or ?secret= as fallback).
// - Generates index.html + README.md with Anthropic (fallbacks included).
// - Creates/updates a public GitHub repo named by "task", enables Pages.
// - POSTs to evaluation_url with repo_url, commit_sha, pages_url.
// - Emits STEP logs to Vercel for easy debugging.
...
/* ---------- tolerant body parser: json, text(json), form, + query fallback ---------- */
async function readBodyFlexible(req) {
  ...
  try {
    ...
    // try best-effort JSON parse for text/plain
    try {
...
async function anthropicREADME(brief, repoName) {
  const r = await fetch(ANTHROPIC_API, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `Create a professional README.md for:
...
export default async function handler(req) {
  ...
  // Parse body tolerantly, also allow ?secret= fallback
  ...
  try {
    ...
    const okNotify = await notify(evaluation_url, { email, task, round, nonce, repo_url: repoUrl, commit_sha: commitSha, pages_url: pagesUrl });
    if (!okNotify) throw new Error("Evaluation notification failed after retries");
